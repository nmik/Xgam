...
def build_wbeam(psf, l_, out_file):
    """Calculates the Wbeam(l, E) and return a bivariate slpine.
       ...
       ----------
          psf : spline
             psf bivaiate spline generated by get_psf
          l_  :  array
...
def wbeam_parse(wb_file, l_max=1500):
    """Created to parse the txt file given in output by build_wbeam.
       ...
       -------
          array, array, tensor
             In order: array of the energies, array of the multipoles, 
             tensor of the 2D Wbeam(l, E)
    """
    f = open(wb_file, 'r')
    e_ = []
    l_ = np.arange(l_max)
...
def get_1D_wbeam(wb_file, spectrum_spline, e_min, e_max):
    """Returns the integral Wbeam as a function of the multiple in a certain
       energy interval.
